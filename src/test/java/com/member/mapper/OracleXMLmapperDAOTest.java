...
@Log4j2
public class OracleXMLmapperDAOTest {
	...
	public void read() {
		//dao = new BoardDAO();
		factory = MybatisManager.getInstance();
	...
	public void testGetTime() {
		...
		try {
			...
			// @BeforeEach @AfterEach 인식안되는 경우 객체생성 안 될 경우 직접 선언
			SqlSession session = MybatisManager.getInstance().openSession();
			
			String getTime = session.selectOne("member.getTime");
			session.close();
...
//	
	...
	public void testMemberList() {
		...
		try {
  			...
  			List<TMemberVO> vo = session.selectList("member.list");
	...
	public void testNameAndEmailSearch() {
		...
		TMemberVO vo = (TMemberVO) session.selectList("member.list2",map);
